...
export const TopNotationBar: React.FC<TopNotationBarProps> = ({
  ...
  return (
    <header className="w-full bg-[#1c1815] border-b border-[#3d3329] px-3 py-2 flex items-center justify-between gap-2 shadow-md z-30 shrink-0 select-none">
      {/* Brand & Motion Badge */}
      <div className="flex items-center gap-2 shrink-0">
        ...
        <div className="hidden sm:block">
          <h1 className="font-bold text-xs text-[#f4ead5] font-display tracking-wider">
            FANTASTIC DICE
          </h1>
          <p className="text-[9px] uppercase tracking-widest text-[#d4c3a1]/50">3D Mobile</p>
        </div>
      </div>

      {/* Center Notation Bar & Quick Presets */}
      <form onSubmit={handleSubmit} className="flex-1 flex items-center justify-center gap-1.5 max-w-xl">
        <div className="flex items-center bg-[#141210] border border-[#3d3329] rounded-xl px-2 py-1 shadow-inner flex-1 max-w-md">
        ...
        <button
          type="submit"
          disabled={isRolling}
          className="px-4 py-1.5 rounded-xl bg-[#8c7851] hover:bg-[#6d5b3d] active:scale-95 text-white font-bold text-xs uppercase tracking-wider flex items-center gap-1.5 shadow-md transition-all disabled:opacity-50 shrink-0 font-display"
        >
      ...
      <div className="flex items-center gap-1.5 shrink-0">
        ...
        <div className="flex items-center gap-1 px-2 py-1 rounded-lg bg-[#8c7851]/15 border border-[#8c7851]/30 text-[#f4ead5] text-[10px] font-medium">
          ...
          <span className="hidden lg:inline">{shakeEnabled ? 'Shake Active' : 'Shake Off'}</span>
        </div>
      </div>
    </header>
  );
